import React, { useCallback, useRef, useEffect } from 'react'; // Added imports
import {
	View,
	RefreshControl,
	ViewToken,
...
import { FlashListRef } from '@shopify/flash-list'; // Import type
...
import { useHomeFollowingTimeline } from '@/hooks/custom/useHomeFollowingTimeline';
...
const HomeFollowingTab = () => {
	...
	const {
		...
		handleRefresh,
		onActivityLoadMore,
	} = useHomeFollowingTimeline(visibleIndexRef);
	...
	if (activityFlattenData && activityFlattenData.length == 0 && isFetching) {
		return (
			<View className="flex-1 mt-[130px]">
				<TimelineLoading />
				<TimelineLoading />
				<TimelineLoading />
			</View>
...
export default HomeFollowingTab;
